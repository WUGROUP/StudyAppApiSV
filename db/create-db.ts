...
export class CreateDb {
    ...
    public static initDb() {
        ...
        db.serialize(
            () => {
                db.run('CREATE TABLE IF NOT EXISTS summaryTestInfo (id INTEGER, title TEXT, score INTEGER, costTime INTEGER, createdDate TEXT)');
                ...
                db.run('CREATE TABLE IF NOT EXISTS textBookInfo (id INTEGER, title TEXT, courseCount INTEGER)');
            }
        )
        db.close();
    }
